...
The Retail Analytics System is designed to capture and process real-time video feeds from multiple cameras and provide actionable insights through a web dashboard and API endpoints. The system is modular, scalable, and supports both local and distributed deployments.
...
Uses YOLOv8 neural network (Ultralytics) for object detection
...
Prediction: Predict next positions of existing tracks

Association: Hungarian algorithm for matching detections to tracks

Update: Update matched tracks, create new tracks, remove old tracks
...
Async tasks: Allow concurrent processing of multiple cameras and API requests
...
Optional: Redis can be used for distributed deployments for pub/sub messaging, caching, and time-series persistence.
...
Horizontal Scaling: Multiple nodes, each processing a subset of cameras, communicate via message queues (Redis Pub/Sub) to a central aggregation service for the dashboard.

Vertical Scaling: GPU acceleration using CUDA-enabled YOLOv8, optional TensorRT optimization, and batch processing of frames for faster inference.
